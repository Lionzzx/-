<template>
	<Card>
		<Button class="top" type="primary" @click="addedit('add')">添加店铺</Button>
		...
		<Input class="top inpt" search suffix="ios-search" placeholder="店铺名称" v-model="limit.search" @on-search="getList" @on-change="getList" />
		...
		<Modal v-model="show" :title="showtitle" @on-cancel="cancel">
			<Form ref="department" :model="department" :rules="rules" :label-width="80">
				<FormItem prop="name" label="店铺名称"><Input v-model="department.name" placeholder="请输入店铺名称" clearable /></FormItem>
				<FormItem prop="node" label="店铺权限"><Tree :data="authTree" show-checkbox @on-check-change="checkChange" /></FormItem>
...
export default {
	data() {
		return {
			...
			list: [],
			rules: {
				name: [{ required: true, message: '请输入店铺<名称', trigger: 'blur' }],
				node: [{ required: true, type: 'array', min: 1, message: '请选择店铺<权限', trigger: 'change' }],
