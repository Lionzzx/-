<template>
	<Card>
		<Button class="top" type="primary" @click="addedit('add')">添加客服</Button>
		...
		<Input class="top inpt" search suffix="ios-search" placeholder="用户名称" v-model="limit.username" @on-search="getList" />
...
<script>
import { getContactList, getRolesList, deleteRoles, batchdelete, addRoles, editRoles } from '@/api/system.js';
...
export default {
		...
		return {
			...
			columns: [
				...
				},
				{
					...
					align: 'center',
					render: (h, res) => {
						return h('span', res.row.status == 0 ? '禁用' : '正常');
					}
				},
				{
					title: '操作',
	...
	methods: {
		...
		async getList() {
			this.loading = true;
			try {
				const res = await getContactList(this.limit);
